...
class NTT_cpu_omp_old : public BigMulImpl {
public:
    std::string name() const override { return "ntt-cpu-omp-old"; }
...
BigInt NTT_cpu_omp_old::multiply(const BigInt &lhs, const BigInt &rhs) {
...
void NTT_cpu_omp_old::set_root(u32 n) {
    assert((n & (n - 1)) == 0);
...
void NTT_cpu_omp_old::dft(u32 n, field *a) {
    set_root(n);
...
void NTT_cpu_omp_old::idft(u32 n, field *a) {
    set_root(n);
...
static bool _ = [](){
    register_impl("ntt-cpu-omp-old", [](){ return new NTT_cpu_omp_old(); });
    return true;
}();